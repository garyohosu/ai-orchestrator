...
- **主フロー**:
  1. 失敗種別を判定する（`DELIVERY_FAILED` / `FAILED` / `TIMEOUT` / `NO_REPLY` / `RATE_LIMITED`）。認証切れ・対象ファイル不足・破壊的操作・人間判断の必要性など判定できない場合は`HUMAN_REQUIRED`とする。
  2. 一時的な失敗は設定回数まで再試行する（対象AIが未受信の場合のみ）。
  3. 再試行上限、またはAI受信後の失敗が確定した時点で、元メールの送信者UIDへエラー通知メールを送信する。件名に元の依頼IDと状態を含める。
  4. 本文へ次の全項目を記載する（SPEC.md 30章「本文」）: 状態、依頼ID、元メールID、元の送信者UID、処理対象AI名、処理対象UID、失敗段階、失敗理由、CLI終了コード、実行時間、再試行回数、最終試行日時、元メールの状態、推奨する次の対応。RATE_LIMITEDの場合は判定根拠、マスキング済みstdout/stderr末尾、出力ログの相対パス・SHA-256・切捨て有無、担当履歴・引継ぎ回数・代替AIまたは候補なし理由も含める。APIキー・認証トークン・Cookie・パスワード・秘密の環境変数・メール本文全文・プロンプト全文・個人情報を含む絶対パス・標準出力/標準エラーの無制限な全文は含めない（SPEC.md 30章「エラー情報の制限」）。
  5. 対象依頼を終端状態（`FAILED` / `TIMEOUT` / `NO_REPLY` / `DELIVERY_FAILED` / `RATE_LIMITED` / `HUMAN_REQUIRED`）として記録する。
- **代替フロー**:
...
### UC-13 利用制限時に代替AIへ引き継ぐ

- **アクター**: オーケストレーター、代替AI、元メール送信者
- **概要**: CLIアダプターが利用制限を根拠付きで検出した場合、同じAIを再起動せず、設定順の未訪問候補へ同じ依頼IDで引き継ぐ。
- **主フロー**:
  1. stdout/stderrを保存し、マスキング後末尾・SHA-256・判定根拠を記録する。
  2. 状態を`RATE_LIMITED`として永続化する。
  3. `fallback_agents`を順に確認し、未訪問かつ上限未到達の候補を選ぶ。
  4. 引継ぎメールを送信し、担当履歴・引継ぎ回数・チェックポイントを保存する。
  5. 代替AIの未読メール処理を通常のディスパッチとして継続する。
- **代替フロー**:
  - 3a. 候補がない、循環、上限到達、送信失敗、または永続化失敗 → `HUMAN_REQUIRED`として元送信者へ通知する。
- **関連**: SPEC.md 10章、20章、23章、26章、30章、FAILOVER_DESIGN.md
